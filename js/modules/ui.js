...
export function initSectionCollapse() {
  ...
  const eyeIconOpen = `<svg viewBox="0 0 24 24"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>`;
  const eyeIconClosed = `<svg viewBox="0 0 24 24"><path d="M17.94 17.94A10.07 10.07 0 0 1 12 20c-7 0-11-8-11-8a18.45 18.45 0 0 1 5.06-5.94M9.9 4.24A9.12 9.12 0 0 1 12 4c7 0 11 8 11 8a18.5 18.5 0 0 1-2.16 3.19m-6.72-1.07a3 3 0 1 1-4.24-4.24"/><line x1="1" y1="1" x2="23" y2="23"/></svg>`;

  sections.forEach((sec) => {
    const eyebrow = sec.querySelector(".eyebrow");
    if (!eyebrow) return;

    let wrapper = sec.querySelector(".eyebrow-wrapper");
    if (!wrapper) {
      wrapper = document.createElement("div");
      wrapper.className = "eyebrow-wrapper";
      eyebrow.parentNode.insertBefore(wrapper, eyebrow);
      wrapper.appendChild(eyebrow);
    }

    if (wrapper.querySelector(".section-collapse-btn")) return;

    const btn = document.createElement("button");
    btn.className = "section-collapse-btn";
    btn.type = "button";
    btn.setAttribute("aria-label", "Toggle section view");
    btn.setAttribute("title", "Hide section content & borders");
    ...
    btn.addEventListener("click", (e) => {
      ...
      if (isCollapsed) {
        ...
        btn.setAttribute("title", "Show section content & borders");
      } else {
        btn.innerHTML = eyeIconOpen;
        btn.setAttribute("title", "Hide section content & borders");
      }
